...
- Use built-in generics (Python 3.10+): `list[str]`, `dict[str, int]`, `tuple[int, ...]` — **never** `List`, `Dict`,
  `Tuple` from `typing`.
...
- Every function body is checked — annotating only the signature is not enough; mypy will flag errors inside unannotated
  bodies too once the signature is typed.
...
python -m mypy boxes/drawing.py boxes/generators/misc/mygenerator/__init__.py
...
boxes/generators/<category>/<snake_case_name>/
    __init__.py        ← the generator source (class definition)
    <snake_case_name>.svg   ← reference SVG (auto-generated)
    <snake_case_name>.jpg   ← sample image (1200 px wide)
    <snake_case_name>-thumb.jpg  ← thumbnail (200 px wide)
```

Example for a new `MyWidget` generator in the `misc` category:

```
boxes/generators/misc/mywidget/__init__.py
boxes/generators/misc/mywidget/mywidget.svg
boxes/generators/misc/mywidget/mywidget.jpg
...
3. **Compares byte-for-byte** against the reference SVG stored **next to the generator source file** (same stem, `.svg`
   extension, e.g. `boxes/generators/boxes/abox.svg`).
...
```powershell
python scripts/regen_svg.py MyGenerator
```
...
```python
import inspect
from boxes.generators.misc.mygenerator import MyGenerator
...
b.render()
data = b.close()
gen_file = Path(inspect.getfile(MyGenerator))
# New layout: source is __init__.py → SVG uses the folder name
stem = gen_file.parent.name if gen_file.name == "__init__.py" else gen_file.stem
out = gen_file.parent / f"{stem}.svg"
...
```powershell
boxes --examples                # writes each .svg into the generator folder
```
...
The test suite will create a hash-suffixed file inside the generator folder: `<category>/<name>/<name>_<hash8>.svg`.
...
|--------------------------|-------------------------------------|-------------------------|--------------------------|
| `touch-hub`              | `boxes/scripts/ui_touch.py`         | `static/touch.css`      | `genHTMLTouchCSS()`      |
| `touch-args` (generator) | `boxes/scripts/pages/generator.py`  | `static/generator.css`  | `genHTMLGeneratorCSS()`  |
| `touch-cat` (categories) | `boxes/scripts/pages/categories.py` | `static/categories.css` | `genHTMLCategoriesCSS()` |
| `touch-colors`           | `boxes/scripts/pages/colors.py`     | `static/colors.css`     | `genHTMLColorsCSS()`     |
| `touch-machine`          | `boxes/scripts/pages/machine.py`    | `static/machine.css`    | `genHTMLMachineCSS()`    |

Rules:

- `touch.css` contains only `:root` CSS variables and `body.touch-hub` styles (header bar, tab bar, generator
  grid/cards).
- Every page Python file declares the relevant `genHTMLXxxCSS()` as `raise NotImplementedError` stub; the real
  implementation lives in `ui_touch.py`.
- Adding a new page requires: new CSS file in `static/`, new `genHTMLXxxCSS()` in `ui_touch.py`, stub in the page mixin.
- **Never use inline `<style>` blocks** in Python page renderers.

---

## 11. Providing Photos / Sample Images  <!-- was §10 -->

- File: `boxes/generators/<category>/<name>/<name>.jpg` (1200 px wide, ~square)
- Thumbnail: `<name>-thumb.jpg` in the same folder (generated by `gen_sample_images.py`)
- Compress with [TinyPNG](https://tinypng.com/) first.
- Run `python scripts/gen_sample_images.py GeneratorName` to regenerate from the SVG.
- Commit: `boxes/generators/<category>/<name>/<name>*.jpg`
